...
function controlSummary(control = {}) {
    ...
    return active.length ? `Issuer intervention is possible through ${active.join(', ')}.`
        : allKnownOff ? 'No listed override is active in the observed mint recipes.'
...
function assetHtml(tokens) {
    ...
    if (!items) return '<li>No current Solana mint is recorded for this programme.</li>';
...
<article><small>What do you own?</small><strong>${esc(grades.claimLabel, 'Claim not established')}</strong><p>${esc(firstSentence(issuer.holderClaim))}</p></article>
<article><small>Can you redeem?</small><strong>${yesNo(redemption.available)}</strong><p>${esc(firstSentence(redemption.eligibility || redemption.rails))}</p></article>
<article><small>Can the issuer intervene?</small><strong>${controlSummary(issuer.control)}</strong><p>Control is reported as observed powers, not collapsed into a score.</p></article>
<article><small>Backing verification</small><strong>${esc(issuer.custodyVerification?.type, 'Not established')}</strong><p>${esc(firstSentence(issuer.custodyVerification?.notes))}</p></article>
</div></section>
