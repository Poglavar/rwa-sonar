// Pure records and static HTML for one observed exact-mint protocol integration.  This does not
// decode accounts or simulate transactions: those proof stages stay false until a collector does it.
...
export function buildProtocolDossiers({ tokens = [], issuers = [], usage = {}, templates = [] }) {
        ...
        for (const [number, integration] of (item.integrations ?? []).entries()) {
            ...
            rows.push({
                ...
                integration, proof: {
                    ...
                    configurationDecoded: proof.configurationDecoded === true, readOnlyExecutionSimulated: proof.readOnlyExecutionSimulated === true,
...
export function renderProtocolDossier(dossier, { baseUrl = null, version = '' } = {}) {
    ...
    const markets = (i.markets ?? []).map((m) => `<li><strong>${text(m.name, 'Market / pool')}</strong><br><code>${text(m.marketAddress ?? m.reserveAddress ?? m.vaultAddress ?? m.bankAddress ?? m.collateralConfig ?? m.address, 'No address recorded')}</code>${m.debtMint ? `<br>Debt mint: <code>${text(m.debtMint)}</code>${m.debtSymbol ? ` (${text(m.debtSymbol)})` : ''}` : ''}</li>`).join('') || '<li>No market-level configuration was recorded.</li>';
