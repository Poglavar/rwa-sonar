...
export function renderProtocolDossier(dossier, { baseUrl = null, version = '', ogImage = null, schematics = [] } = {}) {
    ...
    return `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1">${siteNav.themeScriptHtml('../')}${protocolHead({ dossier, canonical, description, proofModel, ogImage })}${contactStylesheet('../')}<link rel="icon" type="image/svg+xml" href="../images/variant3.svg"><link rel="stylesheet" href="../app-shell.css?v=${encodeURIComponent(version)}"><link rel="stylesheet" href="../protocols.css?v=${encodeURIComponent(version)}"><link rel="stylesheet" href="../flow-diagram.css?v=${encodeURIComponent(version)}"></head><body>${siteNav.siteHeaderHtml('../')}<main>
...
export function renderProtocolIndex(dossiers, { baseUrl = null, version = '', ogImage = null } = {}) {
    ...
    return `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1">${siteNav.themeScriptHtml('../')}${protocolIndexHead({ canonical, count: dossiers.length, ogImage, baseUrl })}${contactStylesheet('../')}<link rel="icon" type="image/svg+xml" href="../images/variant3.svg"><link rel="stylesheet" href="../app-shell.css?v=${encodeURIComponent(version)}"><link rel="stylesheet" href="../protocols.css?v=${encodeURIComponent(version)}"></head><body>${siteNav.siteHeaderHtml('../')}<main><a class="back" href="../stocks.html?view=defi">← DeFi research</a><p class="eyebrow">Exact-address evidence only</p><h1>Protocol and market dossiers</h1><p class="lead">One shareable record per observed integration. Unperformed decoding and simulation steps remain explicit.</p><div class="index">${rows}</div></main>${contactFooterHtml('../')}</body></html>`;
}
