...
export function renderProtocolDossier(dossier, { baseUrl = null, version = '' } = {}) {
    ...
    const markets = (i.markets ?? []).map((m) => {
        const marketKey = m.marketAddress ?? m.reserveAddress ?? m.vaultAddress ?? m.bankAddress ?? m.collateralConfig ?? m.address ?? null;
