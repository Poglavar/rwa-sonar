...
async function main() {
 ...
 const out = resolve(ROOT, flags['out-dir'] ?? 'protocols'); const [tokens, issuers, usage, templates] = await Promise.all([readJson(join(ROOT, 'stocks-tokens.json')), readJson(join(ROOT, 'stocks-issuers.json')), readJson(join(HERE, 'data/defi-usage.json')), readJson(join(HERE, 'data/composability-templates.json'))]);
 const dossiers = buildProtocolDossiers({ tokens: tokens.tokens, issuers: issuers.issuers, usage, templates: templates.templates }); await mkdir(out, { recursive: true });
