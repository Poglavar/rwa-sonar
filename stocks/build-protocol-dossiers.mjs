...
async function main() {
 ...
 const keep = new Set(['index.html', 'index.json']); for (const dossier of dossiers) { keep.add(`${dossier.slug}.html`); keep.add(`${dossier.slug}.json`); await writeFile(join(out, `${dossier.slug}.html`), renderProtocolDossier(dossier, { baseUrl: flags['base-url'], version: '20260923a' })); await writeJson(join(out, `${dossier.slug}.json`), dossier, 0); }
 await writeFile(join(out, 'index.html'), renderProtocolIndex(dossiers, { baseUrl: flags['base-url'], version: '20260923a' })); await writeJson(join(out, 'index.json'), dossiers.map(({ slug, symbol, mint, issuer, integration, proof }) => ({ slug, symbol, mint, issuer, protocol: integration.protocolName, actions: integration.actions, proofStage: proof.sourceStatus })), 0);
