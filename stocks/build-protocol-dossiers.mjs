...
async function main() {
 ...
 const keep = new Set(['index.html', 'index.json']); for (const dossier of dossiers) { keep.add(`${dossier.slug}.html`); keep.add(`${dossier.slug}.json`); const ogImage = absoluteImage(origin, await familyOgImage(og, dossier.slug, protocolOgModel(dossier))); await writeFile(join(out, `${dossier.slug}.html`), renderProtocolDossier(dossier, { baseUrl: flags['base-url'], version: '20260923a', ogImage, schematics })); await writeJson(join(out, `${dossier.slug}.json`), dossier, 0); }
