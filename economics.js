...
(function (root, factory) {
    ...
    function renderProfile(profile, { productSymbol = null, compact = false } = {}) {
        ...
        const content = `<section class="costs"><h3>What the holder pays</h3>${missingLayers}${gap ? `<p class="scope-warning"><strong>${esc(gap.label)}.</strong> ${esc(gap.note)}</p>` : ''}${sections || '<p class="unknown-block">No cost schedule has been reviewed here. This does not mean the product is free.</p>'}<p class="total-note">No all-in total: route, eligibility, current quotes and unreviewed costs can change the result.</p></section><section class="actors"><h3>Who gets paid—and why it matters</h3><p class="section-note">An actor map, not proof that every participant or payment has been identified.</p>${(profile.actors || []).map(actor => actorRow(profile, actor)).join('')}</section><details class="gaps"><summary>What remains unknown <span>${(profile.gaps || []).length}</span></summary><ul>${(profile.gaps || []).map(gap => `<li>${esc(gap)}</li>`).join('')}</ul><p>Research gaps are not accusations of missing disclosure.</p></details>`;
    ...
    async function start(doc, win) {
        ...
        try {
            ...
            coverage.textContent = `${initialCount} of ${data.profiles.length} programmes have initial fee research. The rest remain explicit gaps.`;
