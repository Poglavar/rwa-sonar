...
- Find more attestation types from descriptions of tokenization projects and add them to the attestation-types.json file, introduce recipes.

- Crucially, develop a framework for breaking out freeform counsel opinions into narrower, focused, boolean attestations.

- For real world assets there is a difference between ledgerized, nonledgerized and tokenized (which implies ledgerized). If we tokenize a nonledgerized asset, we automatically ledgerize it.

- Attestation can be issuer or third party, valid or expired, onchain or offchain, public or secret.

- For an asset that is tokenized on multiple chains, do we need a report for each chain? Probably yes.
