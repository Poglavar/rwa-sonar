...
(function (root, factory) {
    ...
    function floatSectionHtml(float, width) {
        ...
        const conflicts = float.porConflicts?.length
            ? `<p class="fl-finding"><strong>The float is an upper bound.</strong> For ${esc(fmtNumber(float.porConflicts.length))} of ${esc(fmtNumber(float.porCompared))} tokens the issuer's own all-chain circulating supply (proof-of-reserves feed) is smaller than what sits outside the issuer-attributed wallets on Solana alone. Tokens held by wallets our research has not attributed — issuer-side, exchange or not-yet-activated inventory — therefore count as float here; the issuer-reported column is the tighter figure where it exists.</p>` : '';
